<label class="field__label" {!! $field->attributes($field->getOption('label_attr')) !!}>{{ $field->getOption('label')
    }}</label>
@php
    /** @var Themosis\Forms\Contracts\FieldTypeInterface $field  */
    $name = $field->getOption('multiple') ? $field->getName().'[]' : $field->getName();
@endphp
<select class="form-control" name="{{ $name }}"
        id="{{ $field->getAttribute('id') }}" {!! $field->attributes($field->getAttributes()) !!}>
